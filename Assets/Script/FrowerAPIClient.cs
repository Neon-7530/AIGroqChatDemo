...
/// 3. 会話の履歴を保存する
/// </summary>
public class GroqChatClient1 : MonoBehaviour
{
...
"話せる最大の文字数は100文字以内にする。\n"+
 ...
 "3. buy_itemは情報を与えるワード（白髪,同じ制服を着ている,眼鏡）をユーザーが言った時のみ入れてください\n" +
 "4. 情報を与えるワード（白髪,同じ制服を着ている,眼鏡）が言われていない場合は空文字列(\"\")を入れてください";
        // 設定を会話履歴に追加
    ...
    private void HandleValidResponse(CustomGroqResponse response)
    {
        ...
        if (!string.IsNullOrEmpty(response.buy_item))  // 商品を購入した場合
        {
            ...
            Debug.Log($"「{response.buy_item}」で手がかりを得られました！");  // デバッグ用に購入を表示
